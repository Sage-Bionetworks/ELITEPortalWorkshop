...
p <- ggplot(data = df, aes(x=variable, y=value)) + 
  geom_boxplot(aes(fill=treatment)) + theme(axis.text.x = element_text(angle = 60, hjust = 1))
...
#check for all metabolites and generate p-values
...
print(mets.names[which(qvals_table$Interaction<=0.05)])
```
...
require(ggplot2)
p1 <- ggplot(data = df.merge.subset.rshpe, aes(x=variable, y=value)) + 
  geom_boxplot(aes(fill=treatment)) + theme(axis.text.x = element_text(angle = 60, hjust = 1))
